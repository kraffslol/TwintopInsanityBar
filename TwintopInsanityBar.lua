...
local function ConstructOptionsPanel()
	...
	local maxWidth = mfloor(GetScreenWidth())
	local minWidth = mmax(mceil(settings.bar.border*2), 120)
	
	local maxHeight = mfloor(GetScreenHeight())
	local minHeight = mmax(mceil(settings.bar.border*2), 1)
	local barWidth = 250
	local barHeight = 20
	local title = ""

	local maxBorderHeight = mmin(mfloor(settings.bar.height/2), mfloor(settings.bar.width/2))
	...
	controls.width:SetScript("OnValueChanged", function(self, value)
		...
		RepositionInsanityFrameThreshold()
		local maxBorderSize = mmin(mfloor(settings.bar.height/2), mfloor(settings.bar.width/2))
	...
	controls.borderWidth:SetScript("OnValueChanged", function(self, value)
		...
		local minBarWidth = mmax(settings.bar.border*2, 120)
		local minBarHeight = mmax(settings.bar.border*2, 1)
